...
void main() {
  ...
  setUp(() async {
    tempDir = await Directory.systemTemp.createTemp('pomodoro-repository');
  ...
  test(
    'creates deterministic daily summary ids per user and day',
    () async {
  ...
  test(
    'logs movement events without disturbing hydration counters',
    () async {
      await repository.loadOrCreateDailySummary(
        dayKey: '2026-01-01',
        openedAt: DateTime.utc(2026, 1, 1, 9),
      ...
      expect(event.type, WellnessEventType.movement);
      expect(summary.movementCount, 1);
      expect(summary.hydrationCount, 0);
    },
    skip: _runPowerSyncDatabaseTests ? false : _powerSyncDatabaseSkipReason,
  );
}
