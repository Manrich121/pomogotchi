...
import 'package:pomogotchi/features/pomodoro/domain/services/session_engine.dart';
...
class PomodoroController extends ChangeNotifier {
  ...
  Future<void> stopSession() async {
    ...
    final refreshedSummary = await _repository.refreshDailySummary(
      dayKey: _todayDayKey,
      now: now,
    );
    _completionStatus = null;
  ...
  Future<void> resetCurrentDay() async {
    final now = _clock.now();
    await _repository.resetCurrentDay(dayKey: _todayDayKey, now: now);
    _completionStatus = null;
    await initialize();
  }
  ...
  Future<void> _logWellnessEvent(WellnessEventType type) async {
    ...
    try {
      ...
      _setState(_state.copyWith(dailySummary: refreshedSummary));
    } finally {
